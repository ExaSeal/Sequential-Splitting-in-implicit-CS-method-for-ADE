...
use_schemes = [
    ...
    (CNCS_ADA_recipe, "CNCS ADA", "CNCS"),
    (CNCS_DAD_recipe, "CNCS DAD", "CNCS"),
...
for recipie, phi_label, compaired_against in use_schemes:
    if compaired_against != 0:
        ...
        error_split = (model_error[compaired_against] - model_error[phi_label])
...
for recipie, phi_label, compaired_against in use_schemes:
    ...
    fig.colorbar(
        ...
        shrink=1,
        aspect=20 * 0.7,
        label="Splitting Scheme Error ($\ell2$ error norm)",
    )
    ...
    plt.tight_layout()
    filename = f"{phi_label.replace(' ', '_')}_split_error_quadrants.png"
